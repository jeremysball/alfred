...
---
### 🎨 Formatting Output in the TUI

The Alfred TUI supports **two** formatting systems that can be used together:

#### 1. Markdown (Preferred for structure)
Use markdown for document structure and semantic formatting:
...
1. Numbered lists
2. For steps
3. Or ordered sequences
```

**Use markdown when:**
- Explaining code (use code blocks)
- Listing items (bullet/numbered lists)
- Emphasizing key terms (**bold**)
- Quoting file names or commands (`code`)
- Creating structured responses

#### 2. Rich Console Markup (For colors only)
Use Rich markup for colors that markdown cannot provide:

```
[cyan]Blue-green text[/cyan]
[red]Red for errors or warnings[/red]
[green]Green for success indicators[/green]
[bold]Bold weight (but prefer **markdown** for this)[/bold]
```

**Valid Rich style names:** `cyan`, `red`, `green`, `blue`, `magenta`, `yellow`, `bold`, `italic`, `dim`, `underline`, `strike`, `reverse`, plus `bright_*` and `on_*` variants.

**Use Rich markup when:**
- Showing success/error status with color
- Highlighting CLI commands in cyan
- Adding color that markdown cannot provide
- **Never use for structure** (lists, code blocks, bold emphasis)

#### Combining Both

```markdown
I found the issue in `config.py`:

**Error:** [red]Missing required field[/red]
...
Next steps:
1. Run [cyan]uv run alfred[/cyan]
2. Test the connection
```

**Rules:**
1. **Prefer markdown** for structure and semantics
2. **Use Rich markup sparingly** - only for colors
3. **Never use invalid tags** like `[1` - they render as plain text
4. **Tag pairs must match:** `[cyan]...[/cyan]` works, `[cyan]...[/blue]` does not
...
### 0. Commit Early, Commit Often — ALWAYS USE THE COMMIT SKILL

**⛔ CRITICAL**: Before making ANY commit, you **MUST** read the commit skill:
...
**NO EXCEPTIONS.** Every commit requires reading the skill first. The skill contains essential patterns for atomic commits, conventional commit format, and recovery procedures.

Make small, atomic commits frequently. Never batch multiple features into one commit.
