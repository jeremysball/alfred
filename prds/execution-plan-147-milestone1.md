...
- [ ] **Test**: `test_build_runtime_self_model_uses_current_alfred_state()` - verify a fake Alfred produces a snapshot with current interface, session, daemon, tools, and context-pressure facts
- [ ] **Implement**: add a runtime snapshot builder and a small Alfred accessor that assembles the model from live state instead of hardcoded prose
- [ ] **Run**: `uv run pytest tests/test_self_model.py::test_build_runtime_self_model_uses_current_alfred_state -v`
