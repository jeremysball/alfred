...
| 2026-03-27 | **IMPLEMENTED**: WhichKey header path contract | Extended `src/alfred/interfaces/webui/static/js/features/keyboard/test-which-key.js` to verify the overlay header shows the active leader path (`Leader + S`) when a submenu path is active. This keeps the breadcrumb vocabulary aligned with the derived tree before the renderer refactor. |
| 2026-03-27 | **IMPLEMENTED**: WhichKey renderer refactor to tree-only state | Updated `src/alfred/interfaces/webui/static/js/features/keyboard/which-key.js` to store the derived tree directly, drop the compatibility `register()` path, and keep path traversal inside the renderer over already-derived data. This completes the pure-renderer slice without changing the existing leader fallback text. |
...
| 2026-03-27 | **DECIDED**: Theme palette remains a nested leader action | Keep theme selection under the registry-backed `Theme` submenu so the canonical browser path is `Ctrl+S -> T -> T`. Browser validation should follow the existing nested chord path rather than flattening the menu or introducing a separate one-key alias. This keeps the theme palette aligned with the leader registry and avoids duplicating access outside the declared shortcut hierarchy. |
...
| 2026-03-27 | **IMPLEMENTED**: Leader runtime dispatcher consumes the registry-derived tree | Updated `src/alfred/interfaces/webui/static/js/main.js` to build leader state from `KeymapManager.buildLeaderTree(getKeymap())`, traverse with `getLeaderNodeForPath()`, and dispatch only through explicit action handlers. This removes the hardcoded leader tree from runtime dispatch and keeps leader mode mirrored to the registry-backed schema. |
| 2026-03-27 | **IMPLEMENTED**: Leader browser coverage now validates runtime parity and invalid chords | Added `tests/webui/test_leader_keybinds.py::test_leader_dispatch_tracks_registry_path_updates`, `test_invalid_leader_key_exits_mode_without_dispatching`, and the frontend popup regression to verify live registry rebinding, submenu legend rendering, and fail-fast exit on unknown chords. |
| 2026-03-27 | **IMPLEMENTED**: Help sheet parity with shared leader breadcrumb | Updated `src/alfred/interfaces/webui/static/js/features/keyboard/help.js` to render the shared leader breadcrumb as secondary text, stay subscribed to keymap changes, and keep help output aligned with `WhichKey`. Added `tests/webui/test_leader_keybinds.py::test_help_sheet_lists_the_same_chord_paths_as_which_key` to prove the help sheet and overlay use the same chord-path vocabulary and reject the legacy `Ctrl+A` leader alias. |
| 2026-03-27 | **IMPLEMENTED**: WhichKey theme surface tokens and panel skinning | Updated `src/alfred/interfaces/webui/static/js/features/keyboard/styles.css` to consume shared panel surface tokens, added defaults in `themes.css` and `base.css`, and updated `modern-dark.css`, `kidcore-playground.css`, `spacejam-neocities.css`, and `elegant-modern-yellow.css` to provide distinct leader-overlay surface values. Added browser coverage in `tests/webui/test_theme_palette.py` and `tests/webui/test_theme_persistence.py` to verify the overlay responds to theme token changes and that targeted theme files declare the expected panel tokens. |
| 2026-03-27 | **IMPLEMENTED**: Theme palette browser validation follows the nested leader path | Updated `tests/webui/test_theme_palette.py` and `tests/webui/test_theme_persistence.py` so the palette opens through `Ctrl+S -> T -> T`, the theme input receives focus before typing, and the Space Jam selection test now uses an unambiguous `space jam` query. This closes the final validation gap without flattening the leader hierarchy or changing runtime behavior. |
