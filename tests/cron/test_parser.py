...
class TestGetNextRun:
    ...
    def test_every_minute(self):
        """* * * * * returns next minute."""
        from_time = datetime(2026, 2, 18, 10, 30, 0, tzinfo=UTC)
        result = get_next_run("* * * * *", from_time)

        assert result == datetime(2026, 2, 18, 10, 31, 0, tzinfo=UTC)

    def test_every_5_minutes(self):
        """*/5 * * * * returns next 5-minute boundary."""
        from_time = datetime(2026, 2, 18, 10, 32, 0, tzinfo=UTC)
        result = get_next_run("*/5 * * * *", from_time)

        assert result == datetime(2026, 2, 18, 10, 35, 0, tzinfo=UTC)
    ...
    def test_every_hour(self):
        ...
        assert result == datetime(2026, 2, 18, 11, 0, 0, tzinfo=UTC)
    ...
    def test_daily_at_time(self):
        ...
        assert result == datetime(2026, 2, 18, 19, 0, 0, tzinfo=UTC)
    ...
    def test_daily_next_day(self):
        ...
        assert result == datetime(2026, 2, 19, 19, 0, 0, tzinfo=UTC)
    ...
    def test_weekly_sunday(self):
        ...
        # Sunday Feb 22, 2026
        assert result == datetime(2026, 2, 22, 19, 0, 0, tzinfo=UTC)
    ...
    def test_weekdays(self):
        ...
        # Monday Feb 23, 2026
        assert result == datetime(2026, 2, 23, 9, 0, 0, tzinfo=UTC)

    def test_list_of_minutes(self):
        """0,30 * * * * returns next :00 or :30."""
        from_time = datetime(2026, 2, 18, 10, 15, 0, tzinfo=UTC)
        result = get_next_run("0,30 * * * *", from_time)

        assert result == datetime(2026, 2, 18, 10, 30, 0, tzinfo=UTC)

    def test_crosses_hour_boundary(self):
        """0,30 * * * * crosses to next hour correctly."""
        from_time = datetime(2026, 2, 18, 10, 45, 0, tzinfo=UTC)
        result = get_next_run("0,30 * * * *", from_time)
    ...
    def test_year_boundary(self):
        ...
        from_time = datetime(2026, 12, 31, 10, 0, 0, tzinfo=UTC)
        result = get_next_run("0 0 31 12 *", from_time)

        assert result == datetime(2027, 12, 31, 0, 0, 0, tzinfo=UTC)

    def test_leap_year_feb_29(self):
        """0 0 29 2 * handles Feb 29 in leap year."""
        # 2028 is a leap year
        from_time = datetime(2028, 2, 1, 10, 0, 0, tzinfo=UTC)
