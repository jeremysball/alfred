...
const { register, getAll, getAllFlat, getById, unregister, clear, formatShortcut, parseKeyCombo } = require('./shortcuts.js');
...
test('getAll groups shortcuts by category', () => {
  ...
  register({ id: 'cmd3', key: 'c', description: 'Action 3', category: 'Navigation', action: () => {} });
...
test('formatShortcut formats special keys', () => {
  assert(formatShortcut({ key: 'arrowup', ctrl: false, shift: false, alt: false, meta: false }) === '↑');
  assert(formatShortcut({ key: 'escape', ctrl: false, shift: false, alt: false, meta: false }) === 'Esc');
  assert(formatShortcut({ key: 'home', ctrl: false, shift: false, alt: false, meta: false }) === 'Home');
});
