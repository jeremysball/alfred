...
test('pull triggers websocket reconnect', async () => {
  ...
  const result = initializePullToRefresh(element, {
    connectionMonitor: mockConnectionMonitor
  });

  assert(result !== null, 'Expected initializePullToRefresh to return object');
  assert(result.detector !== null, 'Expected detector to be created');

  // Simulate pull that triggers refresh
  result.detector._handleTouchStart(createTouchEvent(100, 100, element));
  result.detector._handleTouchMove(createTouchEvent(100, 190, element));

  // Trigger the refresh callback
  await result.detector.onRefresh({});
...
test('reconnect success shows connected state', async () => {
  const mockConnectionMonitor = {
    reconnect: async () => {
  ...
  assert(result.indicator.state === 'success', 'Expected indicator to show success state');

  result.cleanup();
});

test('reconnect failure shows error state', async () => {
  const mockConnectionMonitor = {
    reconnect: async () => {
  ...
  assert(result.indicator.state === 'error', 'Expected indicator to show error state');
