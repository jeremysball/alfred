...
} = require("./coordinated-detectors.js");
...
test("CoordinatedSwipeDetector creates wrapped detector", () => {
  ...
  assert(
    ...
    "Expected coordinator to be GestureCoordinator instance",
  );
});
...
test("axis locking - cannot switch axis after lock established", () => {
  ...
  assert(
    detector.axisLock === "horizontal",
    "Expected axisLock to remain horizontal after attempted switch",
  );
...
test("region coordination - region info stored in gesture metadata", () => {
  ...
  assert(activeGesture !== null, "Expected active gesture info");
  assert(
    activeGesture.region === "message",
    `Expected region 'message', got '${activeGesture.region}'`,
  );
...
test("region coordination - default region for unknown elements", () => {
  ...
  unknownElement.matches = () => false;
  unknownElement.closest = () => null;
